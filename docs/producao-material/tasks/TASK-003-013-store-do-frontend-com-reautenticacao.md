...
**2ª volta (retry `e75a11f` — re-review do gate 8 REPROVOU por REGRESSÃO aberta pelo retry; teto de convergência 4.88 → decisão autônoma do Tech Lead pelo default do reviewer, veto do Diretor na Entrega)**:
- **S1b (gate 8, ALTA — regressão)**: `PUBLIC_AUTH_PATHS` inclui `/auth/logout`, mas `POST /auth/logout` é rota **protegida** no backend (não está em `PUBLIC_PATH_ALLOWLIST`, a fonte que o comentário do bloco cita como canônica). Com access expirado + refresh vivo: "Sair" → 401 → a guarda devolve sem renovar → o handler de `logout` **nunca roda** → família de refresh não revogada, cookies não limpos, cache sobrevive; a próxima requisição autenticada ressuscita a sessão. Correções:
  - remover `/auth/logout` de `PUBLIC_AUTH_PATHS` — o espelho é **só** `/auth/login` + `/auth/refresh` (= `PUBLIC_PATH_ALLOWLIST` do backend);
  - `onQueryStarted` do `logout`: na **falha**, logout **local fail-closed** — `dispatch(api.util.resetApiState())` + redirect para `/login` —, **nunca** engolir; logout que falha jamais deixa a aba num estado autenticado;
  - corrigir o comentário de `PUBLIC_AUTH_PATHS`: 2 dos 3 (agora 2 de 2) caminhos vêm da allowlist do backend; se algum entrar por decisão de cliente, o comentário diz isso e o teste de divergência o exclui da comparação.
...
- [ ] **[retry S1b — 2ª volta]** `POST /auth/logout` com 401 renova a sessão e repete o logout (não é no-op) — verificação executável: `npm --prefix mnemonicos-frontend test -- api` → `POST /auth/logout` responde **401** na 1ª chamada e **204** na 2ª → `POST /auth/refresh` chamado **1×**, `POST /auth/logout` chamado **2×**, `store.getState().api.queries` **vazio** ao fim (a família foi revogada no servidor e o cache zerado). Mutante: manter `/auth/logout` em `PUBLIC_AUTH_PATHS` → `refreshCalls === 0` e `logoutCalls === 1` (o logout vira no-op — vermelho). `Tests: ≥1 passed`. Fixada antes do código.
- [ ] **[retry S1b]** logout que **falha de verdade** (sessão morta) é fail-closed local — verificação executável: `npm --prefix mnemonicos-frontend test -- api` → `POST /auth/logout` 401 **e** `POST /auth/refresh` 401 → `api.util.resetApiState()` dispatchado, redirect para `/login` chamado, `store.getState().api.queries` **vazio**; a aba **não** fica num estado autenticado. Mutante que faz o `onQueryStarted` do `logout` engolir a falha (sem `resetApiState`/redirect) → o cache sobrevive e não há redirect (vermelho). `Tests: ≥1 passed`. Fixada antes do código.
- [ ] **[retry S1b]** `PUBLIC_AUTH_PATHS` não diverge da fonte canônica — verificação executável: `npm --prefix mnemonicos-frontend test -- api` → asserção de que `PUBLIC_AUTH_PATHS` é **exatamente** `['/auth/login','/auth/refresh']` (os caminhos que `PUBLIC_PATH_ALLOWLIST` do backend expõe como públicos); `/auth/logout` **não** está na lista. Mutante que adiciona `/auth/logout` → o teste fica vermelho. `Tests: ≥1 passed`. Fixada antes do código.
