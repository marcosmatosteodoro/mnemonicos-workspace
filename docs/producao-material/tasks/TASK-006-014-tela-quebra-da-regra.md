...
- [ ] **[Design] `EMENDA R3 (pós 2ª rodada de gate 11)` — navegação de volta no cabeçalho da página**: `content/[id]/breakdown/page.tsx` ganha `Link` "Conteúdos brutos" → `/content` no cabeçalho (ao lado do `<h1>`, mesmo padrão das demais páginas da família) — além do "Ir para o Conteúdo bruto" já existente no componente (destinos diferentes: um para o Conteúdo bruto específico, outro para a listagem geral; os dois rotulados por destino).
- [ ] **[Design] `EMENDA R3` — espaçamento consistente**: remover o `p-6` duplicado do ramo de carregando de `rule-breakdown-form.tsx` (mesma correção já aplicada em `content-list.tsx`).
- [ ] **[Design] `EMENDA R3` — validação por campo, não só agregada no rodapé**: mover o padrão de `content-form.tsx` (um `<p role="alert">` por campo, `id` próprio no `aria-describedby` do `<textarea>`) para os 6 campos de `rule-breakdown-form.tsx` — hoje a recusa só aparece consolidada no rodapé, divergindo do irmão da mesma wave. Manter a lista consolidada também, se for mais barato (AC-005-022 pede a listagem); somar a marcação por campo.
- [ ] **[Design] `EMENDA R3` — hint de opcional no `aria-describedby`**: o texto "(deixe vazio se não se aplica)" de CONDIÇÃO/EXCEÇÃO ganha `id` e entra no `aria-describedby` do respectivo `<textarea>` (combinado com o `id` do erro quando houver) — hoje é `<p>` solto, invisível a leitor de tela.
- [ ] **[Testes] `EMENDA R4 (escalada — 3ª rodada de gate 1-7)` — regressão de prova 4.174 no `role="alert"` da recusa consolidada**: o retry anterior trocou `screen.findByRole('alert')` por `screen.findByText(/preencha os campos obrigatórios/i)` em `rule-breakdown-form.test.tsx` para resolver ambiguidade (a marcação por campo criou vários `role="alert"`) — mas isso descartou a prova de que a recusa consolidada É anunciada como alert. Corrigir SEM reintroduzir ambiguidade: `within(container).getByRole('alert', { name: ... })` ou `getAllByRole('alert')` + localizar pelo texto entre eles. Fechamento comparativo (mutante que remove `role="alert"` da recusa consolidada): tem que morrer tanto no commit anterior a este retry quanto no novo — não pode sobreviver só aqui. Fixada antes do código.
- [ ] **[Design] `EMENDA R4 (escalada — 4ª rodada de gate 11)` — token `--danger` aplicado aqui também**: ver EMENDA equivalente em TASK-006-012 — trocar `text-red-500` deste arquivo por `text-danger`.
