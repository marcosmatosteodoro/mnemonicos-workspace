...
**EMENDA Wave 6 (furo no plano herdado da Wave 5 — DEC-012-011, PLAN-012)**: o contrato HTTP mudou depois que TASK-012-010 foi implementada. `GET /contents/:id/strip` deixou de ser get-or-generate (achado de CSRF, security-engineer) — agora é leitura pura, 404 se a Tira ainda não foi aberta; a geração migrou para `POST /contents/:id/strip`. O endpoint RTK Query existente em `mnemonicos-frontend/src/store/api.ts` (`openMnemonicStrip: build.query`, hook `useOpenMnemonicStripQuery`) ainda assume o contrato ANTIGO — precisa ser ajustado no MESMO diff desta TASK (TASK-012-010 está Done e não será reaberta só para isto, decisão já registrada no card de TASK-012-008).

## Escopo

### Inclui

- **EMENDA Wave 6 (DEC-012-011)** — em `mnemonicos-frontend/src/store/api.ts`: renomear o endpoint de leitura existente `openMnemonicStrip` (query) para `getMnemonicStrip` (mesma URL `GET /contents/:id/strip`, mesmo `providesTags: ['MnemonicStrip']` — só o nome muda, para refletir que é leitura pura agora); adicionar um endpoint NOVO `openMnemonicStrip: build.mutation<MnemonicStrip, { rawContentId: string }>` fazendo `POST /contents/:id/strip` (sem corpo), `invalidatesTags: ['MnemonicStrip']`. Exportar `useGetMnemonicStripQuery` e `useOpenMnemonicStripMutation` (ao lado dos hooks já exportados das 4 mutations de Quadro, que não mudam).
...
- `mnemonicos-frontend/src/components/mnemonic-strip-board.tsx` — `'use client'`. Fluxo de abertura (EMENDA acima): `useGetMnemonicStripQuery({ rawContentId })` primeiro; em 404 (Tira ainda não aberta), dispara `useOpenMnemonicStripMutation()` automaticamente (a UI preserva o comportamento de FR-011-001/002 — "abrir" continua sendo um ato único do ponto de vista do usuário, mesmo que agora sejam 2 chamadas HTTP client-side: GET falha 404 → POST gera → re-render com os dados da mutation) — mostra o mesmo indicador de "em andamento" da leitura enquanto a mutation de abertura está pendente; erro da mutation de abertura usa o mesmo padrão de estado de falha das demais ações. As 4 mutations de Quadro (`useAddMnemonicFrameMutation`/`useUpdateMnemonicFrameMutation`/`useRemoveMnemonicFrameMutation`/`useReorderMnemonicFramesMutation`) não mudam. Tipos `MnemonicStrip`/`MnemonicFrame` de `@/types/domain` (COMP-012-009). Lista de Quadros ordenada por `position`; controles simples de mover posição (para cima/para baixo — sem drag-and-drop, A-011-009); os 3 estados observáveis por ação (em andamento — controle desabilitado + indicador visível —, sucesso, falha); estado de "Tira vazia" com ação de criar Quadro disponível quando `frames.length === 0`, sem regeneração automática; cores de erro/sucesso usando os tokens `--danger`/`--link` do tema (`text-danger`/`text-link`), nunca classe literal de paleta.
- `mnemonicos-frontend/src/components/mnemonic-strip-board.test.tsx` — montado com `makeStore()` + `Provider` + `fetch` mockado (mesmo harness de `rule-breakdown-form.test.tsx`), nunca hook extraído isolado. Inclui o caso do fluxo GET-404→POST-gera (abaixo).
- `mnemonicos-frontend/src/store/api.test.ts` (se existir; senão, arquivo de contrato equivalente já usado pelas outras mutations) — ajustar/estender para o endpoint renomeado + o novo, mesmo padrão dos demais.
