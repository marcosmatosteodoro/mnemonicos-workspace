...
- [ ] **[retry S1 — 2ª volta]** A guarda do último ADMIN fecha **na escrita** (não check-then-act), provada **na fronteira da invariante** — verificação executável: `npm --prefix mnemonicos-backend run test:integration -- users.integration.test.ts` → teste com fixture de **exatamente 2 ADMINs ativos** (o penúltimo — não 3+ nem "desativa todos"), que dispara **N ≥ 3 `disableUser` concorrentes** (`Promise.allSettled`) do **mesmo alvo ADMIN** contra o Postgres real; ao fim: `count(User where role='ADMIN' and disabledAt is null) >= 1` **sempre**, e **exatamente 1** chamada resolve — as demais rejeitam com `ConflictError`. **Fecho falsificável**: aplicar o corpo check-then-act de `efaef57` (`count` fora do `$transaction`) e rodar **o comando acima** (arquivo inteiro, **nunca** `-t "..."` isolado — o mutante morre isolado e sobrevive no arquivo, decisão 4.186) → **vermelho** (`adminsAtivosRestantes` chega a 0 / >1 resolve); reverter. Idem para o mutante `isolationLevel: 'Serializable'` → `'ReadCommitted'`. `Tests: ≥1 passed`. Fixada antes do código.
- [ ] **[retry — par de precedência de `disableUser`]** (perfil §7, "Árvore de decisão com precedência declarada"; lição da Wave 2 deste PLAN) — verificação executável: `npm --prefix mnemonicos-backend run test:integration -- users.integration.test.ts` → caso `disableUser(<conta ADMIN **já desativada**>)` com a base tendo **exatamente 1 ADMIN ativo** (≠ o alvo) → resolve como **no-op**, `disabledAt` do alvo **inalterado** (preserva a marca original, não re-carimba), **nenhum** `ConflictError`; o nome do `it` enuncia a precedência ("conta já desativada vence a guarda do último ADMIN → no-op"). Mutante que troca a ordem dos ramos `disabledAt !== null` (return) e `role === 'ADMIN' && activeAdmins <= 1` (`ConflictError`) → o caso fica vermelho (retornaria 409 sobre conta já inativa). `Tests: ≥1 passed`. Fixada antes do código.
...
- [ ] **[retry F5]** `revokeAllSessionsOp` é o único ponto de revogação em massa **por usuário** — verificação executável (oráculo primário = comportamento): `npm --prefix mnemonicos-backend test -- auth users` → `disableUser`, `resetUserPassword` e `changeOwnPassword` continuam revogando as sessões da conta (provas de AC-002-020/022/029 verdes); um teste unitário afirma que os **três** delegam a `revokeAllSessionsOp` (spy/mock de `revokeAllSessionsOp` chamado 1× por cada, com `userId` correto) — a mutação que faz qualquer um dos três voltar a um `updateMany` próprio deixa esse teste vermelho. Checagem de apoio (não é o oráculo): `rg -n "revokedAt:\s*now" mnemonicos-backend/src/` aponta para exatamente **dois** pontos — `revokeAllSessionsOp` (por usuário, este item) e `revokeFamily` (revogação por `familyId`, código da Wave 3, fora de escopo desta TASK); nenhum outro. `Tests: ≥1 passed`. Fixada antes do código.
