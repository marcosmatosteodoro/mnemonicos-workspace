...
- `mnemonicos-backend/src/modules/users/users.routes.ts` — `usersRoutes: Router` com as 4 rotas, **cada uma** com `requireRole(<método>, <caminho completo>, 'ADMIN')` como middleware da própria rota (assinatura da EMENDA Wave 4 — nunca `usersRoutes.use(requireRole(...))` cego no topo): `GET /users`, `POST /users`, `PATCH /users/:id/disable`, `POST /users/:id/reset-password`. Sem rota de registro público. Sem `try/catch`.
...
3. `users.routes.ts` — `requireRole(<método>, <caminho completo>, 'ADMIN')` por rota (assinatura da EMENDA Wave 4); nenhum caminho de registro.
