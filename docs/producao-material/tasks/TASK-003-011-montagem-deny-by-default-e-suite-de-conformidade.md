...
COMP-003-016 + COMP-003-019, DEC-003-005 (+ EMENDA da Wave 4), §1.3 da SPEC (métrica de sucesso) e §9 da DoD do PLAN. É **a** barreira: `requireAuth` montado em `apiRoutes` **antes** de qualquer router de módulo e **depois** das rotas públicas, e todo router não-público declara seus papéis via **`requireRole(method, path, ...roles)`** (assinatura método-aware da EMENDA — declara em `ROUTE_ROLES` na avaliação da chamada, tempo de montagem), de modo que uma rota adicionada sem declaração nasça negada (falha fechada, 403 mesmo com sessão válida). Após montar toda a árvore, `routes.ts`/`createApp()` chama **`sealRouteRoles()`** — declaração posterior lança. Carrega o item de métrica: a suíte de conformidade que enumera as rotas montadas e prova 401/403 para cada não-pública — fonte de medição externa da §1.3.
...
### Inclui
- `mnemonicos-backend/src/http/routes.ts` — `apiRoutes` monta nesta ordem: `healthRoutes` (público); rotas públicas de `authRoutes` (`/auth/login`, `/auth/refresh`); `apiRoutes.use(requireAuth)`; então `authRoutes` protegidas com `requireRole('<MÉTODO>', '<caminho-completo>', 'EDITOR','ADMIN')` (`/auth/logout`, `/auth/change-password`, `/auth/me`), `usersRoutes` com `requireRole('<MÉTODO>', '<caminho-completo>', 'ADMIN')` por rota, `disciplinesRoutes` com `requireRole('GET', '/disciplines', 'EDITOR','ADMIN')`. Cada `requireRole` recebe o **método e o caminho completo visto por `requireAuth`** (a partir da raiz de `apiRoutes`) — árvore montada plana, sem `apiRoutes.use('/prefixo', subRouter)`, para que `req.path` case a chave declarada. `/disciplines` passa a exigir sessão **e** declaração de papel (A-002-019; SPEC §4.1.9). Ordem conferida em `createApp()`; **`sealRouteRoles()` ao fim da montagem**.
- `mnemonicos-backend/tests/integration/route-authz-matrix.test.ts` — deriva a lista de rotas de `apiRoutes.stack` (Express 5 removeu `app._router`), com **método + caminho completo** de cada rota; para cada caminho ∉ `PUBLIC_PATH_ALLOWLIST`: 401 sem cookie; para cada caminho sob `requireRole(...,'ADMIN')`: 403 com sessão de `EDITOR`; o caso da rota fictícia montada **depois** de `requireAuth` e **sem** `requireRole` roda **com sessão de EDITOR** e espera **403** (não 401) — falha fechada por ausência de declaração em `ROUTE_ROLES` (AC-002-014). Assert de snapshot: `PUBLIC_PATH_ALLOWLIST` é **exatamente** `['/health','/health/db','/auth/login','/auth/refresh']` (crescer exige mudança deliberada). Assert global: nenhum handler montado que crie `User` (`prisma.user.create` ou o service de criação) está fora de um router com `requireRole(...,'ADMIN')` registrado em `ROUTE_ROLES` (AC-002-018, além de `usersRoutes`). Oráculo capaz de falhar: remover `requireAuth` da montagem, ou remover a declaração de `/disciplines` de `ROUTE_ROLES`, torna o teste vermelho.
...
1. Reordenar `routes.ts`: públicas → `apiRoutes.use(requireAuth)` → routers protegidos, cada rota com `requireRole('<MÉTODO>', '<caminho-completo>', ...papéis)` (árvore plana); `disciplinesRoutes` passa para depois do `requireAuth` com `requireRole('GET', '/disciplines', 'EDITOR','ADMIN')`. `sealRouteRoles()` ao fim.
2. Suíte de matriz: enumerar `{ método, caminho-completo }` de `apiRoutes.stack`, iterar, asserção 401/403 por rota; asserção de **chave exata** em `ROUTE_ROLES` para toda rota montada (+ caso adversarial da irmã estática); caso da rota fictícia sem declaração (403 com sessão de EDITOR); snapshot da allowlist; assert global de criação de `User`.
...
- [ ] **[herdado — EMENDA DEC-003-005 / resíduo do retry de TASK-003-007]** Toda rota montada tem declaração **exata `<MÉTODO> <caminho-completo>`** em `ROUTE_ROLES` — fecha o vazamento em que `rolesForPath` casa uma rota irmã estática não declarada contra o `:param` do vizinho do mesmo método — verificação executável: `npm --prefix mnemonicos-backend test -- route-authz-matrix` → para **cada** `{ método, caminho-completo }` de `apiRoutes.stack` fora de `PUBLIC_PATH_ALLOWLIST`, afirma que `ROUTE_ROLES` tem a **chave exata** `"<MÉTODO> <caminho>"` (igualdade, não `rolesForPath` com fallback de padrão); rota montada sem a chave exata → o teste **falha** (e, se a montagem tiver um passo de asserção de boot, `createApp()` lança). Prova adversarial: montar `GET /_probe/export` (estática) ao lado de uma rota `:param` declarada permissiva (`GET /_probe/:id` com `['EDITOR','ADMIN']`) **sem** declarar a irmã → sessão EDITOR recebe **403** (não herda o vizinho). Mutante que reprova: trocar a asserção de chave exata por `rolesForPath(metodo, caminho) !== undefined` → a irmã não declarada passa a "coberta" e o caso adversarial vira 200. `Tests: ≥1 passed`. Fixada antes do código.
