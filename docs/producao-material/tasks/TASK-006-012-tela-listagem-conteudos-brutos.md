...
- [ ] **[Design] `EMENDA pós gate 11` — ação primária de registro alcançável por clique**: o cabeçalho de `content/page.tsx` ganha um `Link` "Novo conteúdo bruto" → `/content/new` (mesma escala do `<h1>`, ao lado dele); a mensagem de vazio de `content-list.tsx` repete o mesmo controle. Sem isso a orientação do vazio ("registre o primeiro conteúdo") não tem afford ância na tela. Verificação executável: `npm --prefix mnemonicos-frontend test -- content-list` → `getByRole('link', { name: /novo conteúdo bruto/i })` presente tanto com lista vazia quanto com itens. Falsificável: remover o link → o teste não acha o role (vermelho).
