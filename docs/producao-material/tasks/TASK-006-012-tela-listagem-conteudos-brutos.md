...
- [ ] **[Testes] `EMENDA R4 (escalada — 3ª rodada de gate 1-7)` — 3º estado da condição de ação única**: `content/page.test.tsx` monta hoje só os estados vazio e com-itens para provar "1 só ação primária". A condição da EMENDA original é "em QUALQUER estado" — acrescentar o 3º caso nomeado (estado de erro, `isError` de `useListRawContentsQuery`) com a MESMA asserção de contagem (`getAllByRole('link', {name: /novo conteúdo bruto/i})` → `toHaveLength(1)`). Mutante: reintroduzir o `Link` no ramo `isError` de `content-list.tsx` → o novo caso fica vermelho. Fixada antes do código.
- [ ] **[Design] `EMENDA R4 (escalada — 4ª rodada de gate 11)` — token `--danger` para contraste de erro**: `text-red-500` (mensagens de erro/falha, botão "Remover") mede 3.56-3.82:1 no tema claro — abaixo do piso. Criar `--danger: var(--color-red-700)` em `:root` e `--danger: var(--color-red-400)` no bloco dark de `globals.css` (mesmo padrão de `--link`), expor `@utility text-danger`, e trocar as ocorrências de `text-red-500` nas 3 telas da wave (`content-list.tsx`, `content-form.tsx`, `rule-breakdown-form.tsx`) por `text-danger`. Valores conferidos: red-700 ≈6:1 no claro, red-400 ≈6.5:1 no escuro — não usar red-600 (falha nos dois temas).
- [ ] **[Design] `EMENDA R4` — CTA do cabeçalho num degrau intermediário**: o link "Novo conteúdo bruto" do cabeçalho de `page.tsx` hoje usa `text-2xl font-semibold tracking-tight`, idêntico ao `<h1>` ao lado — dois títulos na mesma tela. Trocar para um degrau intermediário (`text-base font-medium text-link underline`), mantendo `text-2xl font-semibold` só no `<h1>`. Não usar o padrão de botão `surface-card px-4 py-2` (colidiria visualmente com o "Sair" do shell na mesma borda).
- [ ] **[Design] `EMENDA R4` — limpeza cosmética (opcional)**: o wrapper `flex flex-col items-start gap-3` do estado vazio de `content-list.tsx` ficou sem função após a remoção dos links duplicados — pode simplificar para o `<p>` direto, sem obrigação.
