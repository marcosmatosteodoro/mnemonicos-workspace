...
**Interface pública**: `decideRefresh(session: SessionRow, now: Date, graceSeconds: number): RefreshDecision`, onde `RefreshDecision` é união discriminada: `{ kind: 'rotate' }` | `{ kind: 'replay-grace' }` (reapresentação do token imediatamente anterior dentro da graça → tratar como `rotate` sem penalizar: nova ponta, sem revogação nem `token.reuse`) | `{ kind: 'reuse' }` (token já rotacionado além da graça, ou `revokedAt` setado → revogar família) | `{ kind: 'expired' }` (`refreshExpiresAt < now`). Ordem de avaliação dos ramos: **expired → reuse → replay-grace → rotate** (ramos coincidentes seguem essa precedência — expiração absoluta vence a graça). Não toca banco, não lê relógio.
**Dependências**: nenhuma
...
- `refresh(refreshToken: string, ctx: { ip: string; userAgent?: string }, now: Date): Promise<IssuedSession>` — o `ctx` carrega o indicador de origem exigido por NFR-002-005 para os eventos de auditoria (o `src/lib/audit.ts` da Wave 2 fixou `ip` como obrigatório em `AuthAuditEvent`; a rota passa `{ ip: req.ip, userAgent: req.get('user-agent') }` — COMP-003-010). Acha a linha por `refreshTokenHash`; delega a `decideRefresh` (COMP-003-006); em `rotate` marca `rotatedAt = now` na linha atual, cria sucessora no mesmo `familyId` **em transação** (`prisma.$transaction`) e emite `recordAuthEvent({ type: 'token.refresh', ... })`; em `replay-grace` **reemite uma nova ponta da família** (mesmo caminho de `rotate` — não idempotente; sem revogação nem `token.reuse` dentro da graça): reemitir o sucessor já criado é infeasível sob DEC-003-002 (só hashes persistidos, o valor em claro não é recuperável), então dentro da graça a reapresentação apenas gera uma ponta nova, sem penalizar; em `reuse` faz `revokedAt = now WHERE familyId`, emite `recordAuthEvent({ type: 'token.reuse', ... })` e lança `UnauthorizedError`; em `expired` lança `UnauthorizedError` — **e se `rotatedAt != null || revokedAt != null`** também `revokedAt = now WHERE familyId` + `recordAuthEvent({ type: 'token.reuse', ... })` (gate 8 da Wave 2). Token ausente ou linha não encontrada → `UnauthorizedError` genérico (mesma `message` do login inválido), nunca exceção não tratada.
...
<!-- EMENDA Wave 4 (ver DEC-003-005): `rolesForPath(req.method, req.path)` — a consulta é por `"<MÉTODO> <caminho>"`; `requireAuth` nega quando o conjunto é `undefined` **ou não contém `req.auth.role`** (piso de autorização), auditando `authz.denied` nesse ramo; precedência 401 (sem sessão) antes de 403. -->
...
<!-- EMENDA Wave 4 (ver DEC-003-005): assinatura efetiva `requireRole(method: HttpMethod, path: string, ...roles: UserRole[])` — a declaração acontece na **avaliação da chamada** (tempo de montagem) via `declareRouteRoles(method, path, roles)`, NUNCA dentro do handler de request; o guard de runtime fica como defesa em profundidade. `sealRouteRoles()` sela o registro pós-boot. -->
**Dependências**: COMP-003-005
...
<!-- EMENDA Wave 5 (retry — gate 8, ALTA): a guarda do último ADMIN é **check-then-act** e uma corrida de 2 `disableUser` concorrentes zera os ADMINs ativos (lockout irreversível — reativação está fora de F1). A decisão fecha **na escrita**: `prisma.$transaction(async (tx) => { count DENTRO da tx; <= 1 → ConflictError; update `disabledAt` + `revokeAllSessionsOp` }, { isolationLevel: 'Serializable' })`, capturando a falha de serialização do Postgres como `ConflictError` (fail-closed). **Nota da implementação (27ef0dd)**: nesta stack (Prisma 7 + `@prisma/adapter-pg`) a falha 40001/40P01 chega como `DriverAdapterError` (`message === 'TransactionWriteConflict'`), **não** como `PrismaClientKnownRequestError` P2034; o guard (`isTransactionWriteConflict`) casa as duas formas. Alternativa aceitável: `UPDATE ... WHERE ... AND (SELECT count(*) ...) > 1` via `$executeRaw` (template tag) com `rowsAffected === 0` como a recusa. Prova exigida (decisão 4.177): teste de integração com N desativações **concorrentes** do penúltimo ADMIN contra o Postgres real, contando linhas no fim (`count(role=ADMIN, disabledAt=null) >= 1` sempre). Padrão já no repo: `createInternalUser` (P2002), `rotateFamilyTip` (CAS). -->
...
<!-- EMENDA Wave 6 (retry S1 — gate 8): a máquina de re-autenticação **exclui os endpoints públicos de sessão** (`PUBLIC_AUTH_PATHS` = `['/auth/login','/auth/refresh']`, espelho literal de `PUBLIC_PATH_ALLOWLIST` do backend, com teste de divergência) — 401 de `POST /auth/login` (credencial inválida) **não** dispara `refresh`. `logout` e `login` fazem `onQueryStarted` → `resetApiState()` (após sucesso E falha). --><!-- EMENDA Wave 7 (furo no plano — AC-002-027 × S1b): o `catch` do `logout.onQueryStarted` **não força navegação**. O redirect da "sessão morta de vez" (401 de `logout` + `refresh` também 401) já vive no ramo `!renewed` do `baseQueryWithReauth`; falha **transitória** de `logout` (500/rede, sessão ainda válida) → só `resetApiState()`, e o `InternalShell` mostra a mensagem genérica e o usuário **permanece** na área interna (FR-002-023). -->
<!-- EMENDA Wave 7 (retry — gate 1 code-reviewer, 2ª volta; TETO 4.88 batido → decisão do Tech Lead em nome do Diretor, veto na Entrega): o "só `resetApiState()`" da linha acima **era o defeito** — em falha transitória de `logout` ele apaga `me`, o `InternalShell` cai no ramo `isLoading`/`!data`, **desmonta** `LogoutControl` e destrói o `useState` `hasFailed`; a mensagem de falha nunca chega ao DOM (AC-002-027 verde na suíte, quebrado em produção — o oráculo de função pura não montava o componente). Correção: `logout.onQueryStarted` faz `resetApiState()` **apenas após sucesso** (idêntico a `login.onQueryStarted` — higiene de cache por aba só quando a identidade de fato terminou); falha transitória → **nem reset nem navegação**, a sessão pode seguir válida e o usuário mantém seu cache e vê a mensagem para repetir. A "sessão morta de vez" continua zerando o cache e redirecionando pelo `setTimeout` do ramo `!renewed` do `baseQueryWithReauth` (COMP-003-008/DEC-003-010, intacto). Com `me` preservado na falha transitória, a guarda `isUninitialized`/`isFetching` que a 1ª volta do retry pôs em `computeMissingSession` fica sem propósito → `InternalShell` volta ao predicado inline `missingSession = isError || (!isLoading && !data)` e o export sai (a menos que o oráculo montado mostre necessidade residual). Prova exigida: teste com `InternalShell` **montado** contra a `api` real (store real + `fetch` do realm Node via `testEnvironment` custom que estende `jest-environment-jsdom` — sem dependência nova), `logout` 500 → a mensagem "Não foi possível sair agora. Tente novamente." **permanece** no DOM e `router.replace` **não** é chamado; mutante = `resetApiState()` no `finally` → a mensagem some (desmontagem) → vermelho. -->
...
<!-- EMENDA Wave 6 (retry — gate 1/4/5): o `config.matcher` **enumera os prefixos internos** explicitamente (`(interno)` é route group, não aparece na URL — as páginas sob ele geram URLs limpas; working set `['/studio/:path*','/gestao/:path*']`, confirmado contra `src/app/(interno)/` e estendido por TASK-003-015 quando criar telas). **Nunca** catch-all por exclusão (`/((?!login|_next/...).*)`) — isso guarda a home pública `/` e inverte o default do site. O teste do matcher afirma os **dois** lados: as rotas internas são guardadas **e** `/` (e toda rota pública existente) **não** é. -->
**Dependências**: nenhuma
...
**Renovação.** `POST /auth/refresh` → lê `mnemo_refresh` → acha linha por `refreshTokenHash` → `decideRefresh(session, now, AUTH_REFRESH_GRACE_SECONDS)`: `rotate` → transação: `rotatedAt = now` na linha atual + nova `Session` no mesmo `familyId`, reemite cookies; `replay-grace` → reemite **uma nova ponta da família** (não idempotente por construção — DEC-003-002 só persiste hashes; sem revogação nem `token.reuse` dentro da graça — renovações concorrentes do SPA não deslogam, AC-002-026; o cliente serializa o refresh, TASK-003-013/DEC-003-010); `reuse` → `revokedAt = now WHERE familyId` + 401; `expired` → 401 (exige novo login — FR-002-005).
...
<!-- EMENDA (gate 7 da Wave 3, 2026-08-29 — decisão do Tech Lead em nome do Diretor, veto na Entrega): a cláusula "responde idempotente / reemite cookies do sucessor já criado" no `replay-grace` é **infeasible sob esta mesma DEC** (só hashes são persistidos; o valor em claro do sucessor não é recuperável). Comportamento adotado: `replay-grace` = tratar como `rotate` (nova ponta da família em transação) **sem** revogar nem auditar `token.reuse` — a reapresentação do token imediatamente anterior dentro da graça não é penalizada. AC-002-026 ("ambas recebem credencial válida, família não revogada") e A-002-018 (janela de graça) seguem satisfeitos. Consequência propagada a TASK-003-013: o `baseQueryWithReauth` serializa o refresh (promessa única em voo) para que duas renovações não corram no cliente. -->
...
<!-- EMENDA (gates 8 + 1/5/7 da Wave 4, 2026-08-29 — decisão do Tech Lead em nome do Diretor, veto na Entrega; a condição "necessidade de papel por método HTTP na mesma rota" do **Reabrir se** desta DEC disparou). Três correções ao mecanismo, todas realizadas em COMP-003-011/012 (TASK-003-007, commit ead57a2):
 (1) o registro `ROUTE_ROLES` é chaveado por **`"<MÉTODO> <caminho>"`**, não só por caminho — `GET /x` e `DELETE /x` com papéis distintos coexistem; `rolesForPath(method, path)` casa `:param` apenas entre chaves do MESMO método. Onde este bloco e a §7/§4 dizem "o registro é por caminho, não por método HTTP", leia-se "por método + caminho".
 (2) a declaração acontece na **avaliação de `requireRole(method, path, ...roles)`** (tempo de montagem), NUNCA por efeito colateral dentro do handler de request — na cadeia Express `requireAuth` roda antes do guard de `requireRole`, então declarar no handler chega tarde e o deny-by-default degenera em deny-tudo. `sealRouteRoles()` sela o registro após o boot (`declareRouteRoles` posterior lança); `resetRouteRoles()` limpa e dessela (testes/remontagem).
 (3) `requireAuth` é **piso de autorização**: nega quando `rolesForPath(...)` é `undefined` OU quando o conjunto não contém `req.auth.role` (antes checava só a existência), com `recordAuthEvent('authz.denied')` também nesse ramo. `requireRole` fica como restrição adicional por rota (defesa em profundidade). Precedência: 401 (sem sessão) antes de 403 (não declarado / papel errado).
 Resíduo conhecido, fechado por COMP-003-019 (TASK-003-011): `rolesForPath` ainda casa uma rota IRMÃ ESTÁTICA não declarada contra o `:param` do vizinho do mesmo método (fail-closed enquanto o papel não estiver no conjunto do vizinho; vizinho permissivo vazaria). A suíte de conformidade exige declaração exata `método + caminho-completo` para TODA rota montada e falha o boot caso contrário — é o fechamento pleno. -->
<!-- EMENDA Wave 6 (retry — gate 8, MEDIA; é a lição registrada após a Wave 4 finalmente aplicada): a dimensão de método alcança também a **allowlist de exceção**. `PUBLIC_PATH_ALLOWLIST` passa a ser pares `"<MÉTODO> <caminho>"` (`['GET /health','GET /health/db','POST /auth/login','POST /auth/refresh']`); `isPublicPath(method, path)` com igualdade exata; `assertDenyByDefault` (COMP-003-016) filtra por método+caminho **e** exige que toda rota cujo par está na allowlist esteja montada **antes** de `requireAuth` na `apiRoutes.stack`. Sem isso, `GET /auth/login`/`DELETE /health` etc. atravessam `requireAuth` sem sessão e o boot não reclama (latente hoje — 404; real no 1º método extra montado num desses 4 caminhos). Toca `public-paths.ts` + `authenticate.ts` (leitor) — carve-out no "Não inclui" da TASK-003-011. -->
**Alternativas consideradas**:
...
**Reabrir se**: surgir uma classe legítima de rota pública que não caiba numa allowlist curta por par `método+caminho` exato.
**Irreversível**: nao
