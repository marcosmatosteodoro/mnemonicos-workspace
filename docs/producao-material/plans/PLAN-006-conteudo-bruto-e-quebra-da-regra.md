...
- [ ] Todos os 24 FRs cobertos têm implementação satisfazendo os ACs
- [ ] Todos os 7 NFRs cobertos têm verificação
- [ ] Decisões DEC-006-001 a DEC-006-009 refletidas no código
- [ ] Aderência à ficha/perfil validada (backend: schema→service→routes, sem Prisma na rota, erro via `AppError`, montagem sob `requireAuth` com `requireRole`/`verifyOrigin`; frontend: Server Component default, estado de servidor só em RTK Query, rótulos pt-BR do mapa de `domain.ts`, `config.matcher` como array literal)
- [ ] Todos os ACs cobertos por teste (gate 1 dos quality gates) — incluindo `contents.integration.test.ts` e a paridade cross-repo estendida
- [ ] Migração Prisma aditiva revisável no diff da TASK de schema; execução confirmada com o Diretor antes de rodar (TRISK-006-001)
- [ ] Métrica da SPEC (§1.3 declara `Fonte de medição`): (a) 100% da superfície nova (7 rotas) no censo da suíte `route-authz-matrix`, com o tripwire atualizado de 12 para 19 pares e **verde no CI**, 0 rota não-declarada; (b) invariante "todo `RawContent` persistido tem `radarClass`" provado por AC-005-002 / AC-005-003 e pela semente (AC-005-027) — nenhum caminho de escrita cria conteúdo sem classe; (c) número **primário observacional** (Conteúdos brutos de Obrigação Tributária com Quebra da regra completa produzidos **pela tela**, mais a razão `registrados : com-quebra`) apurado por inspeção humana e registrado no relatório de Entrega de F2 — **não há componente de instrumentação neste PLAN** (instrumentação de tempo por etapa é F3); fonte externa (suíte de conformidade herdada de F1) + dono (time de engenharia) registrados no INDEX
