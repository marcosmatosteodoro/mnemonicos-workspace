...
|----|-------|--------------|-------|--------|
| PLAN-003 | SPEC-002 | 24/24 FRs + 9 NFRs (autenticação de sessão, autorização deny-by-default, gestão de contas por ADMIN) | 16/16 ✅ | Approved |
...
| HANDOFF-003 | Verificação de tela pendente — HANDOFF-PLAN-003 (login e shell interno; 6 itens V1–V6): trânsito real ao `/studio` no sucesso do login, caminhada e2e de AC-002-013, ida-e-volta UI→servidor do logout de sucesso e da falha transitória | fechar num ambiente com tela (app local + browser + credencial do realm `app`); prompt no relatório da Entrega. **Diretor aceitou a Entrega como PARCIAL (2026-08-31, E1)** — este handoff permanece bloqueio de *onboarding real* (fechar antes do 1º acesso da equipe por browser e antes de F2 depender do login por tela) | qa (gate 9 PARCIAL, causa: credencial) |
| RISK-002-003-audit | `/keelson:audit` sobre o diff de F1 (3 deps novas de cripto/sessão: `@node-rs/argon2`, `cookie-parser`) — DoD do PLAN-003 sem ato registrado (E2 do PO) | ato do Diretor: rodar `/keelson:audit` antes do merge (o assistente não pode invocá-lo) | PO (Entrega F1) |
