...
| HANDOFF-003 | Verificação de tela — HANDOFF-PLAN-003. **V1–V4 OK** (Playwright 2026-09-01). **V5 corrigido** na branch `fix/producao-material-logout-success-race` (BRIEF-004, `9f2225c`·`8f91d3b`·`635314c`; gates 1–9 APROVADO/CONVERGE; flag `justLoggedOut` — logout de sucesso vai a `/login` exato, 0 `refresh`; verificado no browser 2×). **V6** ainda não exercitado na tela. | **Diretor**: revisar + PR + merge da branch do V5 (como F1); depois reexercitar V5+V6 contra o merge e fechar o handoff (`status: Concluído`, tirar este risco, commit `chore(producao-material): close verification handoff HANDOFF-PLAN-003`) | qa (gate 9) + Playwright 2026-09-01 |
...
- 2026-09-01: **BRIEF-004** (avulso, fast-follow do V5) Concluído — branch `fix/producao-material-logout-success-race` (`9f2225c`·`8f91d3b`·`635314c`, **sem push**). Flag `justLoggedOut`: logout de sucesso não dispara "sessão expirou" nem laço de `refresh`. Gates 1–9 APROVADO/CONVERGE (teto 4.88 num gap de briefing → addendum autorizado pelo Diretor; §6.3 do `next-16.md` amendada = 3 oráculos). V5 verificado no browser (Playwright 2×). 1 MEDIA não-bloqueante (dívida): `__resetAuthGuards()` test-hook exportado sem cerca de lint. Falta: merge (Diretor) + caminhada de tela de V5/V6.
